...
├── lib/                  # Feature API functions + response DTO types (api-client wrapper)
...
- `lib/`: API functions and response DTO types. Managed as `{feature}-api.ts` files. See `resources/api-convention.md` for the full convention.
- `types/`: TypeScript interfaces and type definitions.
- `utils/`: All feature-specific logic (formatters, validators, helpers). **Requires >90% test coverage** for custom logic.

> **CRITICAL**: Components must never import `apiGet`/`apiPost`/`apiDelete` directly from `api-client.ts`. All API calls go through `features/{name}/lib/{name}-api.ts`. Read `resources/api-convention.md` before writing any API integration code.
...
- API convention: `resources/api-convention.md`
